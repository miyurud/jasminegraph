...
"""
    ...
    def answer_question(self, question: str) -> str:
        ...
        all_answers = []
        prior_result_text = "Failed to answer"

        for idx, subq in enumerate(subquestions):
            try:
                cypher = self.generate_cypher(subq, context=prior_result_text)
                ...
                prior_result_text = json.dumps(result_data)[:3000]  # Limit context size
                all_answers.append(f"Q{idx+1}: {subq}\nCypher: {cypher}\nA{idx+1}: {result_data}\n")

            except Exception as e:
                all_answers.append(f"Q{idx+1}: {subq}\nA{idx+1}: Failed to answer ({str(e)})\n")

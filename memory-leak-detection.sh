...
/else/,/fi/ {
    /if \[ \$MODE -eq 1 \]; then/,/else/ {
        /valgrind/d
        s|./JasmineGraph|valgrind --leak-check=full --show-leak-kinds=all  --gen-suppressions=all  --show-suppressed=yes --suppressions=./valgrind-suppression.supp ./JasmineGraph|
    }
}
' run-docker.sh

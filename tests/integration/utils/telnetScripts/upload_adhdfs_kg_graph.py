...
HOST = "127.0.0.1"
PORT = 7782  # The port used by the server
...
def send_and_expect_response(conn, test_name, send, expected, exit_on_failure=False):
    ...
    if not expect_response(conn, expected):
...
def test(host, port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        send_and_expect_response(
            sock,
            "initiate-streaming-kg-construction",
            b"initiate-streaming-kg-construction",
            b"ok",
        ...
        send_and_expect_response(
            sock,
            "adhdfs",
            b"192.168.1.7:7780:7781,192.168.1.7:7780:7781",
            b"ok",
            exit_on_failure=True,
        )
        ...
        send_and_expect_response(
                sock,
                "adhdfs",
                b"/home/hotpotqa_full_corpus.txt",
                b"ok",
                exit_on_failure=True,
            )

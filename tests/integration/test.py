...
def test(host, port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        send_and_expect_response(sock, 'cypher',b"match (n) where n.partitionID = '1' return n order by"
                                 ...
                                 exit_on_failure=True)
        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"id":"1","label":"Person","name":"Bob","occupation":"Banker",'
                                 b'"partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"id":"3","label":"Person","name":"David","occupation":"Doctor",'
                                 b'"partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',b'{"n":{"id":"11","label":"Person",'
                                                     b'"name":"George","occupation":"Chef","partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"category":"Restaurant","id":"17","label":"Location",'
                                 b'"name":"Gourmet Bistro","partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"category":"School","id":"5","label":"Location",'
                                 b'"name":"Greenfield School","partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',b'{"n":{"id":"13","label":"Person",'
                                                     b'"name":"Ian","occupation":"Pilot","partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"category":"Coworking Space","id":"19","label":"Location",'
                                 b'"name":"Innovation Hub","partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"category":"Bank","id":"7","label":"Location","name":"Town Bank",'
                                 b'"partitionID":"1"}}', exit_on_failure=True)

        send_and_expect_response(sock, 'cypher', b'',
                                 b'{"n":{"category":"Hospital","id":"9","label":"Location",'
                                 b'"name":"Town General Hospital","partitionID":"1"}}', exit_on_failure=True)
        send_and_expect_response(sock, 'cypher', b'',
                                 b'done', exit_on_failure=True)

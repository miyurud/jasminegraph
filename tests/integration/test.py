...
ADD_STREAM_KAFKA_CSV = b'adstrmkcsv'
ADD_STREAM_KAFKA = b'adstrmk'
EMPTY = b'empty'
RMGR = b'rmgr'
VCNT = b'vcnt'
ECNT = b'ecnt'
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
    ...
    print()
    logging.info("Testing kafka streaming")
    send_and_expect_response(sock, 'Streaming test', ADD_STREAM_KAFKA, b'DO you want to use default KAFKA consumer(y/n) ?', exitOnFail=True)
    send_and_expect_response(sock, "Streaming test", b'y', b'send kafka topic name', exitOnFail=True)
    send_and_expect_response(sock, "Streaming test", b'test_topic', b'Success', exitOnFail=True)

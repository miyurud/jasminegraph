    ...
    if (inputOperator) {
        ...
        if (index == 0) {
            ...
            for (int right = index; right<patternElements.size(); right++) {
                ...
                {
                    inputOperator = new ExpandAll(inputOperator,newStartVar,newDestvar, newRelVar,newRelType);

                } else {
                    auto direction = patternElements[right]->elements[0]->elements[0]->nodeType == Const::LEFT_ARRROW ? "left" : "right";
                    inputOperator = new ExpandAll(inputOperator,newStartVar,newDestvar, newRelVar,newRelType,direction);
                }
    ...
    if (isRelTypeExist) {
        ...
        if ( e->elements[0]->elements[0]->nodeType == Const::UNIDIRECTION_ARROW) {
            if (analyzedDetails.first[0]) {
                inputOperator = new UndirectedRelationshipTypeScan(analyzedDetails.second[1]->elements[0]->value, relVar, startVar, destVar);

            } else {
                inputOperator = new UndirectedRelationshipTypeScan(analyzedDetails.second[0]->elements[0]->value, relVar, startVar, destVar);
            }

        } else {
            auto direction = e->elements[0]->elements[0]->nodeType == Const::LEFT_ARRROW ? "left" : "right";
            if (analyzedDetails.first[0]) {
                inputOperator = new DirectedRelationshipTypeScan(direction,analyzedDetails.second[1]->elements[0]->value, relVar, startVar, destVar);
            } else {
                inputOperator = new DirectedRelationshipTypeScan(direction,analyzedDetails.second[0]->elements[0]->value, relVar, startVar, destVar);
            }
        }

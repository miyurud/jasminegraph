...
private:
  ...
  std::vector<JasmineGraphServer::worker>
      workerList; // List of workers for remote expansion
  std::unordered_map<std::string, std::vector<float>> typeEmbeddingCache;

public:
  SemanticBeamSearch(FaissIndex *faissStore, std::vector<float> emb, int k,
                     GraphConfig gc);
  SemanticBeamSearch(FaissIndex *faissStore, TextEmbedder *textEmbedder,
  ...
  void semanticMultiHopBeamSearch(SharedBuffer &buffer, int numHops,
                                  int beamWidth);
  nlohmann::json callRemoteExpansion(

...
static void aggregate_streaming_centralstore_triangles_command(int connFd,
                                                               std::map<std::string, JasmineGraphIncrementalLocalStore *> &incrementalLocalStoreMap,
                                                               bool *loop_exit_p);
...
string JasmineGraphInstanceService::aggregateCompositeCentralStoreTriangles(std::string compositeFileList,
    ...
    TriangleResult triangleResult =  Triangles::countTriangles(aggregatedCompositeCentralStore, distributionHashMap, true);
    std::string triangles = triangleResult.triangles;
...
bool JasmineGraphInstanceService::sendFileThroughService(std::string host, int dataPort, std::string fileName,
    ...
    if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE) != 0) {
        instance_logger.error("Incorrect response. Expected: " + JasmineGraphInstanceProtocol::SEND_FILE +
                              " ; Received: " + response);
        close(sockfd);
        return false;
    }
    ...
    for (;;) {
        ...
        instance_logger.info("Bytes read " + to_string(nread));
        ...
        if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
            ...
            if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
                ...
                if (Utils::send_str_wrapper(sockfd, std::to_string(partitionID))) {
                    instance_logger.info("Sent : Partition ID " + std::to_string(partitionID));
                }
...
static void in_degree_distribution_command(
    ...
    map<long, long> degreeDistribution = calculateInDegreeDist(graphID, partitionID, serverPort, graphDBMapLocalStores,
...
static void streaming_triangles_command(
    ...
    instance_logger.log("Received oldLocalRelationCount: " + oldLocalRelationCount, "info");
    ...
    instance_logger.log("Received oldCentralRelationCount: " + oldCentralRelationCount, "info");
    ...
    instance_logger.log("Received mode: " + mode, "info");
...
static void aggregate_streaming_centralstore_triangles_command(int connFd, std::map<std::string, JasmineGraphIncrementalLocalStore *> &incrementalLocalStoreMap,
                                                               bool *loop_exit_p) {
    if (!Utils::send_str_wrapper(connFd, JasmineGraphInstanceProtocol::OK)) {
        *loop_exit_p = true;
        return;
...
string JasmineGraphInstanceService::aggregateStreamingCentralStoreTriangles(
        ...
        int threadPriority, std::map<std::string, JasmineGraphIncrementalLocalStore *> incrementalLocalStores, std::string mode) {
    ...
    for (partitionIdListIterator = partitionIdList.begin(); partitionIdListIterator != partitionIdList.end();
        ...
        if (incrementalLocalStores.find(graphIdentifier) == incrementalLocalStores.end()) {
            incrementalLocalStoreInstance =
                    JasmineGraphInstanceService::loadStreamingStore(graphId, aggregatePartitionId, incrementalLocalStores, "app");
        } else {

...
void *instanceservicesession(void *dummyPt) {
    ...
    while (!loop) {
        ...
        if (line.compare(JasmineGraphInstanceProtocol::HANDSHAKE) == 0) {
            ...
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::COLLECT_BLOOM_FILTERS, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
        ...
        else if (line.compare(JasmineGraphInstanceProtocol::INITIATE_CLUSTERING) == 0) {
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::INITIATE_CLUSTERING, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            ...
            //Read partition count from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            int partitionCount = stoi(data);
            graphID = utils.trim_copy(graphID, " \f\n\r\t\v");
            instance_logger.log("Received Graph ID: " + partitionCount, "info");

            //Read cluster count from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            int clusterCount = stoi(data);
            graphID = utils.trim_copy(graphID, " \f\n\r\t\v");
            instance_logger.log("Received Graph ID: " + clusterCount, "info");

            EntityResolver er;
            er.clusterFilters(graphID, partitionCount, clusterCount);

        }
        else if (line.compare(JasmineGraphInstanceProtocol::DISTRIBUTE_CLUSTERS) == 0) {
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::DISTRIBUTE_CLUSTERS, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            ...
            //Parse host cluster details string
            vector<string> partitionLocales = utils.split(hostPartitionListStr, '|');
            //Iterate through each worker partition pair and collect from other workers
            int count = 0;
            std::thread *workerThreads = new std::thread[partitionLocales.size()];
            for (auto p: partitionLocales) {
                ...
                workerThreads[count++] = std::thread(JasmineGraphInstance::sendFileThroughService, hostname, dataPort,
                                                     filename, filepath);
                sleep(1);
            }
            ...
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::BUCKET_LOCAL_CLUSTERS, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            //Read destination host name
            ...
            //Read requested partitions from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string partitionID = (data);
            partitionID = utils.trim_copy(partitionID, " \f\n\r\t\v");
            ...
            EntityResolver er;
            er.bucketLocalClusters(graphID, 256, {stoi(partitionID)});
        }
        ...
        else if (line.compare(JasmineGraphInstanceProtocol::COLLECT_BUCKETS_TO_MASTER) == 0) {
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::COLLECT_BUCKETS_TO_MASTER, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);

            //Read coordinator host name
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string serverHostName = (data);
            serverHostName = utils.trim_copy(serverHostName, " \f\n\r\t\v");
            instance_logger.log("Received HostName: " + serverHostName, "info");
        ...
        else if (line.compare(JasmineGraphInstanceProtocol::COLLECT_BUCKETS_TO_COORDINATOR) == 0) {
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::COLLECT_BUCKETS_TO_COORDINATOR, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            ...
            //Read coordinator host name
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string orgHostListStr = (data);
            orgHostListStr = utils.trim_copy(orgHostListStr, " \f\n\r\t\v");
            instance_logger.log("Received HostName: " + orgHostListStr, "info");
            vector<string> orgHostList = utils.split(orgHostListStr, '|');

            //Read coordinator port from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string orgPortListStr = (data);
            orgPortListStr = utils.trim_copy(orgPortListStr, " \f\n\r\t\v");
            instance_logger.log("Received Port: " + orgPortListStr, "info");
            vector<string> orgPortList = utils.split(orgPortListStr, '|');
            ...
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::SHARE_BUCKETS_TO_COORDINATOR, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
            ...
            //Read coordinator host name
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string coordinatorHost = (data);
            coordinatorHost = utils.trim_copy(coordinatorHost, " \f\n\r\t\v");
            instance_logger.log("Received HostName: " + coordinatorHost, "info");

            //Read coordinator port from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string coordinatorPort = (data);
            coordinatorPort = utils.trim_copy(coordinatorPort, " \f\n\r\t\v");
            instance_logger.log("Received Port: " + coordinatorPort, "info");

            //Read coordinator data port from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string coordinatorDataPort = (data);
            coordinatorDataPort = utils.trim_copy(coordinatorDataPort, " \f\n\r\t\v");
            instance_logger.log("Received Data Port: " + coordinatorDataPort, "info");
            ...
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string orgHost = (data);
            orgHost = utils.trim_copy(orgHost, " \f\n\r\t\v");
            instance_logger.log("Received HostName: " + orgHost, "info");

            //Read coordinator port from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string orgPort = (data);
            orgPort = utils.trim_copy(orgPort, " \f\n\r\t\v");
            instance_logger.log("Received Port: " + orgPort, "info");

            //Read coordinator data port from socket
            bzero(data, INSTANCE_DATA_LENGTH);
            read(connFd, data, INSTANCE_DATA_LENGTH);
            string orgDataPort = (data);
            orgDataPort = utils.trim_copy(orgDataPort, " \f\n\r\t\v");
            instance_logger.log("Received Data Port: " + orgDataPort, "info");
        ...
        else if (line.compare(JasmineGraphInstanceProtocol::DISTRUBUTE_SHARED_CLUSTERS) == 0) {
            instance_logger.log("Received : " + JasmineGraphInstanceProtocol::DISTRUBUTE_SHARED_CLUSTERS, "info");
            write(connFd, JasmineGraphInstanceProtocol::OK.c_str(), JasmineGraphInstanceProtocol::OK.size());
            instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::OK, "info");
            bzero(data, INSTANCE_DATA_LENGTH);
...
int distributeSharedClustersToWorkers(std::string host, int port, int dataPort,
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
        ...
        write(sockfd, JasmineGraphInstanceProtocol::COLLECT_SHARED_CLUSTERS.c_str(),
              JasmineGraphInstanceProtocol::COLLECT_SHARED_CLUSTERS.size());
        instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::COLLECT_SHARED_CLUSTERS, "info");
        ...
        if (line.compare(JasmineGraphInstanceProtocol::SEND_FILE_NAME) == 0) {
            ...
            if (line.compare(JasmineGraphInstanceProtocol::SEND_FILE_LEN) == 0) {
                ...
                line = (data);
                //send content
                if (sockfd.compare(JasmineGraphInstanceProtocol::SEND_FILE_CONT) == 0) {
                    instance_logger.log("Received : " + JasmineGraphInstanceProtocol::SEND_FILE_CONT, "info");
                    instance_logger.log("Going to send file through service", "info");
                    JasmineGraphInstance::sendFileThroughService(host, dataPort, fileName,
                                                                 filePath);
                }
            }
        }
    ...
    JasmineGraphInstance::sendFileThroughService(host, dataPort, fileName, filePath);
}
...
int JasmineGraphInstanceService::collectBucketsToCoordinator(instanceservicesessionargs *sessionargs,
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
        ...
        write(sockfd, JasmineGraphInstanceProtocol::SHARE_BUCKETS_TO_COORDINATOR.c_str(),
              JasmineGraphInstanceProtocol::SHARE_BUCKETS_TO_COORDINATOR.size());
        instance_logger.log("Sent : " + JasmineGraphInstanceProtocol::SHARE_BUCKETS_TO_COORDINATOR, "info");
        ...
        if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
            ...
            instance_logger.log("Sent : " + host, "info");

            //Write org ports to socket
            write(sockfd, to_string(port).c_str(), to_string(port).size());
            instance_logger.log("Sent : " + port, "info");
            ...
            write(sockfd, to_string(clusterCount).c_str(), to_string(clusterCount).size());
            instance_logger.log("Sent : Graph ID " + to_string(clusterCount), "info");

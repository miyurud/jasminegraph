...
void JasmineGraphInstanceService::entityRes(string trainData) {
    ...
    attrFilterPath = utils.getJasmineGraphProperty("org.jasminegraph.entity.location")+"attrfilters_" + graphID + "_" + partitionID + ".txt";
...
//    writeToFile(structFilterPath, structFilters);
 ...
 */
map<int, string> JasmineGraphInstanceService::createFilters(map<int, vector<string>> entityData, int filterSize = 256,
                                                            int numHashes = 4) {
...
JasmineGraphInstanceService::createFilters(map<int, vector<string>> entityData, map<int, vector<int>> neighborhoodData, int filterSize = 256,
              int numHashes = 4) {
    //Create bloom filters
    cout << "Creating filters" << endl;
    Utils utils;
...
void JasmineGraphInstanceService::clusterFilters(string trainData, int noClusters) {
...
void JasmineGraphInstanceService::generateLocalCandidateSets(int filterSize = 256, int noClusters = 3){
    ...
    for (int i = 0; i < noClusters; i++) {
        //Load cluster file into memory
        string filename = utils.getJasmineGraphProperty("org.jasminegraph.entity.location") + to_string(i) +"filters.txt";
        Mat<float> clusterData;
        clusterData.load(filename, arma::csv_ascii);
        //Remove node id column
        clusterData.shed_col(0);

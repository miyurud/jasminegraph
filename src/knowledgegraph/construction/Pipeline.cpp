...
const size_t OVERLAP_BYTES =
    1024;  // bytes to overlap between chunks to avoid splitting lines

const std::string END_OF_STREAM_MARKER = "-1";

Pipeline::Pipeline(int connFd, hdfsFS fileSystem, const std::string& filePath,
                   int numberOfPartitions, int graphId, std::string masterIP,
                   vector<JasmineGraphServer::worker>& workerList,
                   std::vector<std::string> llmRunners,
                   std::string llmInferenceEngine, std::string llm,
                   std::string chunkSize, std::string chunksPerBatch,
                   long startFromBytes)
    : connFd(connFd),
...
void Pipeline::streamFromHDFSIntoBuffer() {
  ...
  kg_pipeline_stream_handler_logger.info(
      "Started streaming data from HDFS into data buffer...");
  ...
  kg_pipeline_stream_handler_logger.info(
      "Total file size: " + std::to_string(total_file_size) + " bytes");
  ...
  kg_pipeline_stream_handler_logger.info("Successfully opened HDFS file: " +
                                         filePath);
  ...
  if (startFromBytes > 0) {
    if (startFromBytes >= total_file_size) {
      kg_pipeline_stream_handler_logger.error(
          "startFromBytes exceeds total file size.");
    ...
    if (hdfsSeek(fileSystem, file, startFromBytes) != 0) {
      kg_pipeline_stream_handler_logger.error(
          "Failed to seek to startFromBytes: " +
    ...
    kg_pipeline_stream_handler_logger.info("Starting read from byte offset: " +
                                           std::to_string(startFromBytes));
  }
  while ((read_bytes = hdfsRead(fileSystem, file, buffer.data(),
                                std::stol(chunkSize))) > 0) {
    if (stopFlag) {
      isReading = false;
      kg_pipeline_stream_handler_logger.info(
          "Recevied Stop command , terminating");
      break;
    }

    int64_t remaining_bytes = total_file_size - bytes_read_so_far;
    double percent_read =
        (static_cast<double>(bytes_read_so_far) / total_file_size) * 100.0;
    ...
    kg_pipeline_stream_handler_logger.info("Starting to process chunk " +
                                           std::to_string(chunk_idx));
    std::string chunk_text = leftover + std::string(buffer.data(), read_bytes);

    kg_pipeline_stream_handler_logger.info(
        "Read chunk " + std::to_string(chunk_idx) + " with " +
        std::to_string(read_bytes) + " bytes");
    kg_pipeline_stream_handler_logger.info("Current leftover size: " +
    ...
    if (last_newline == std::string::npos) {
      kg_pipeline_stream_handler_logger.info("No newline found in chunk " +
                                             std::to_string(chunk_idx) +
                                             ", storing as leftover");
      leftover = chunk_text;
      kg_pipeline_stream_handler_logger.info("Updated leftover size: " +
                                             std::to_string(leftover.size()));
      continue;
    }
    ...
    if (newline_pos != std::string::npos &&
        newline_pos + 1 < full_lines_chunk.size()) {
    ...
    kg_pipeline_stream_handler_logger.info(
        "Full lines chunk size: " + std::to_string(full_lines_chunk.size()));
    kg_pipeline_stream_handler_logger.info("Leftover after split size: " +
                                           std::to_string(leftover.size()));
    kg_pipeline_stream_handler_logger.info(
        "Pushing chunk " + std::to_string(chunk_idx) + " to dataBuffer");

    // Wait and push to dataBuffer safely
    std::unique_lock<std::mutex> lock(dataBufferMutex);
    kg_pipeline_stream_handler_logger.info(
        "Waiting to acquire lock for dataBuffer push");
    dataBufferCV.wait(lock, [this] {
      return dataBuffer.size() < workerList.size() || !isReading;
    });

    dataBuffer.push(
        Chunk{to_string(chunk_idx), std::move(full_lines_chunk), read_bytes});
    kg_pipeline_stream_handler_logger.info(
        "Chunk " + std::to_string(chunk_idx) +
        " pushed to dataBuffer. Current buffer size: " +
        std::to_string(dataBuffer.size()));
    lock.unlock();
    dataBufferCV.notify_all();

    chunk_idx++;
    kg_pipeline_stream_handler_logger.info("Finished processing chunk " +
                                           std::to_string(chunk_idx));
  ...
  if (!leftover.empty()) {
    chunk_idx++;
    kg_pipeline_stream_handler_logger.info(
        "Pushing leftover data to dataBuffer");
    std::unique_lock<std::mutex> lock(dataBufferMutex);
    dataBufferCV.wait(lock, [this] {
      return dataBuffer.size() < workerList.size() || !isReading;
    });
    dataBuffer.push(
        Chunk{to_string(chunk_idx), std::move(leftover), read_bytes});
  ...
  if (!leftover.empty()) {
    kg_pipeline_stream_handler_logger.info(
        "Pushing leftover data again to dataBuffer after closing file");
    std::unique_lock<std::mutex> lock(dataBufferMutex);
    dataBuffer.push(
        Chunk{to_string(chunk_idx), std::move(leftover), read_bytes});
  ...
  {
    kg_pipeline_stream_handler_logger.info(
        "Pushing END_OF_STREAM_MARKER to dataBuffer");
    std::unique_lock<std::mutex> lock(dataBufferMutex);
    // close all workers
    for (auto worker : workerList) {
      dataBuffer.push(
          Chunk{to_string(chunk_idx), END_OF_STREAM_MARKER, read_bytes});
    }
    lock.unlock();
  }
  ...
  kg_pipeline_stream_handler_logger.debug(
      "Successfully streamed data from HDFS into data buffer.");
  kg_pipeline_stream_handler_logger.info(
      "Time taken to read from HDFS: " + to_string(duration.count()) +
      " seconds");
}

void Pipeline::startStreamingFromBufferToWorkers() {
  auto startTime = high_resolution_clock::now();
  // HDFSMultiThreadedHashPartitioner partitioner(numberOfPartitions, graphId,
  // masterIP, isDirected);
  ...
  for (auto& worker : workerList) {
    ...
    workerThreads.emplace_back(
        &Pipeline::extractTuples, this, worker.hostname, worker.port, masterIP,
        graphId, count, std::ref(dataBuffer), std::ref(*bufferPool[count]));
    count++;
  }
  ...
  Utils::sendExpectResponse(connFd, data, INSTANCE_DATA_LENGTH, META.c_str(),
  ...
  if (!Utils::sendIntExpectResponse(
          connFd, ack3, JasmineGraphInstanceProtocol::OK.length(),
  ...
  Utils::sendExpectResponse(connFd, data, INSTANCE_DATA_LENGTH, DONE.c_str(),
                            JasmineGraphInstanceProtocol::OK);

  kg_pipeline_stream_handler_logger.info(
      "Total time taken for streaming from HDFS into partitions: " +
      to_string(duration.count()) + " seconds");
}

json Pipeline::processTupleAndSaveInPartition(
    const std::vector<std::unique_ptr<SharedBuffer>>& tupleBuffer) {
  auto startTime = high_resolution_clock::now();
  kg_pipeline_stream_handler_logger.info(
      "Starting processTupleAndSaveInPartition");
  HDFSMultiThreadedHashPartitioner partitioner(
      numberOfPartitions, graphId, masterIP, true, workerList, true, 5);
  std::hash<std::string> hasher;
  std::vector<std::thread> tupleThreads;
  for (size_t i = 0; i < tupleBuffer.size(); ++i) {
    SharedBuffer* tupleBufferRef = tupleBuffer[i].get();
    kg_pipeline_stream_handler_logger.info(
        "Launching tuple thread for partition " + std::to_string(i));
    tupleThreads.emplace_back([&, tupleBufferRef, i]() {
      int realtimePartitionMetaUpdateIndicator = 0;
      kg_pipeline_stream_handler_logger.info(
          "Tuple thread started for partition " + std::to_string(i));
      while (isProcessing) {
        if (!tupleBufferRef->empty()) {
          realtimePartitionMetaUpdateIndicator++;
          if (realtimePartitionMetaUpdateIndicator != 0 &&
              realtimePartitionMetaUpdateIndicator % 5 == 0) {
            ...
            graph["graph_status_idgraph_status"] =
                Conts::GRAPH_STATUS::OPERATIONAL;
            ...
            string response =
                Utils::read_str_wrapper(connFd, data, FED_DATA_LENGTH);
            // if (response== "stop")
            // {
            //     stopFlag = true;
            // }
            ...
            int converted_number = htonl(message_length);
            kg_pipeline_stream_handler_logger.debug(
                "Sending content length: " + to_string(converted_number));

            if (!Utils::sendIntExpectResponse(
                    connFd, ack3, JasmineGraphInstanceProtocol::OK.length(),
                    converted_number, JasmineGraphInstanceProtocol::OK)) {
              Utils::send_str_wrapper(connFd,
                                      JasmineGraphInstanceProtocol::CLOSE);
              close(connFd);
              return;
            }
            ...
            if (response == "stop") {
              kg_pipeline_stream_handler_logger.info("stop request:" +
                                                     response);
              stopFlag = true;
            }
            lock.unlock();
          ...
          kg_pipeline_stream_handler_logger.debug(
              "Thread " + std::to_string(i) + " processing line: " + line);
          // Check for end-of-stream marker
          if (line == END_OF_STREAM_MARKER) {
            kg_pipeline_stream_handler_logger.debug(
                "Received end-of-stream marker in thread " + std::to_string(i));
            // isProcessing = false;
            break;
          }
          try {
            ...
            kg_pipeline_stream_handler_logger.debug(
                "Thread " + std::to_string(i) + " sourceId: " + sourceId +
            ...
            if (!sourceId.empty() && !destinationId.empty()) {
              ...
              //   std::string edgeId =
              //   std::to_string(hasher(jsonEdge["properties"]["id"])% 10000);
              ...
              kg_pipeline_stream_handler_logger.error(
                  "Malformed line: missing source/destination ID: " + line);
            }
            ...
            kg_pipeline_stream_handler_logger.error(
                "Invalid node ID (not an integer) in line: " + line);
          } catch (const std::out_of_range& e) {
            kg_pipeline_stream_handler_logger.error(
                "Node ID out of range in line: " + line);
          } catch (const std::exception& e) {
            kg_pipeline_stream_handler_logger.error(
                "Unexpected exception in line: " + std::string(e.what()));
          }
        }
        // else if (!isReading) {
        //     kg_pipeline_stream_handler_logger.info("Thread " +
        //     std::to_string(i) + " exiting due to isReading=false"); break;
        // }
      }
      kg_pipeline_stream_handler_logger.info(
          "Tuple thread finished for partition " + std::to_string(i));
    });
  }
  ...
  std::chrono::duration<double> duration = endTime - startTime;
  kg_pipeline_stream_handler_logger.info(
      "processTupleAndSaveInPartition completed in " +
...
void Pipeline::extractTuples(std::string host, int port, std::string masterIP,
  ...
  kg_pipeline_stream_handler_logger.info(
      "Starting extractTuples for host: " + host + ", port: " +
  ...
  kg_pipeline_stream_handler_logger.info("Connected to host: " + host +
                                         " on port: " + std::to_string(port));

  // 1. Perform handshake
  kg_pipeline_stream_handler_logger.info(
      "Performing handshake with masterIP: " + masterIP);
  ...
  kg_pipeline_stream_handler_logger.info(
      "Sending INITIATE_STREAMING_TUPLE_CONSTRUCTION");
  if (!Utils::sendExpectResponse(
          ...
          JasmineGraphInstanceProtocol::OK)) {
    kg_pipeline_stream_handler_logger.error(
        "Failed to initiate streaming tuple construction");
    Utils::send_str_wrapper(sockfd, JasmineGraphInstanceProtocol::CLOSE);
    close(sockfd);
    return;
  }
  kg_pipeline_stream_handler_logger.info(
      "INITIATE_STREAMING_TUPLE_CONSTRUCTION successful");

  // 3. Send graph ID
  kg_pipeline_stream_handler_logger.info("Sending graphID: " +
  ...
  // 3. Send LLM  info
  if (!Utils::sendExpectResponse(sockfd, data, INSTANCE_DATA_LENGTH, llm,
                                 JasmineGraphInstanceProtocol::OK)) {
    kg_pipeline_stream_handler_logger.error("Failed to send LLM runner info");
    Utils::send_str_wrapper(sockfd, JasmineGraphInstanceProtocol::CLOSE);
    close(sockfd);
  ...
  while (true) {
    if (stopFlag) {
      kg_pipeline_stream_handler_logger.info(
          "Received END_OF_STREAM_MARKER for partitionId: " +
      ...
      Utils::send_str_wrapper(sockfd, JasmineGraphInstanceProtocol::CLOSE);
      kg_pipeline_stream_handler_logger.info(
          "Closed connection for partitionId: " + std::to_string(partitionId));
    ...
    std::unique_lock<std::mutex> lock(dataBufferMutex);
    dataBufferCV.wait(lock, [this, &dataBuffer] {
      return !dataBuffer.empty() || !this->isReading;
    });
    ...
    kg_pipeline_stream_handler_logger.info(
        "Processing chunk for partitionId: " + std::to_string(partitionId));
    lock.unlock();
    dataBufferCV.notify_all();
    ...
    // Send chunk
    kg_pipeline_stream_handler_logger.info(
        "Sending QUERY_DATA_START for chunk of size: " +
    ...
    if (!Utils::sendExpectResponse(
            ...
            JasmineGraphInstanceProtocol::OK)) {
      kg_pipeline_stream_handler_logger.error(
          "Failed to send QUERY_DATA_START");
      break;
    }
    ...
    Utils::expect_str_wrapper(sockfd,
                              JasmineGraphInstanceProtocol::GRAPH_DATA_SUCCESS);

    // Receive tuple from server
    kg_pipeline_stream_handler_logger.info(
        "Waiting for QUERY_DATA_START from server");
    if (!Utils::expect_str_wrapper(
            sockfd, JasmineGraphInstanceProtocol::QUERY_DATA_START)) {
      kg_pipeline_stream_handler_logger.error(
          "Did not receive QUERY_DATA_START from server");
      break;
    }
    ...
    while (true) {
      ...
      if (ret <= 0) {
        kg_pipeline_stream_handler_logger.error(
            "Failed to receive tuple length, closing stream");
        break;
      }
      ...
      if (tuple_length <= 0 || tuple_length > 10 * 1024 * 1024) {
        // limit to 10MB
        kg_pipeline_stream_handler_logger.error("Invalid tuple length: " +
                                                std::to_string(tuple_length));
        break;
      }

      kg_pipeline_stream_handler_logger.info(
          "Received tuple length: " + std::to_string(tuple_length) +
          " from: " + std::to_string(partitionId));
      Utils::send_str_wrapper(
          sockfd, JasmineGraphInstanceProtocol::GRAPH_STREAM_C_length_ACK);
      ...
      if (received != static_cast<size_t>(tuple_length)) {
        kg_pipeline_stream_handler_logger.error(
            "Incomplete tuple received, expected " +
      ...
      if (stopFlag) {
        ...
        Utils::send_str_wrapper(
            sockfd, JasmineGraphInstanceProtocol::GRAPH_DATA_SUCCESS);
      }
      ...
      if (tuple_count % 5 == 0) {
        realTimeBytesMutex.lock();
        realtime_bytes_read_so_far +=
            (static_cast<long>(std::stod(chunkSize) * 0.2));
        realTimeBytesMutex.unlock();
        // Utils::sendExpectResponse(connFd, data, INSTANCE_DATA_LENGTH,
        //                                    std::to_string(realtime_bytes_read_so_far),
        //                                    JasmineGraphInstanceProtocol
        //                                    ::OK);
      }
      kg_pipeline_stream_handler_logger.info(
          "Received end of tuple stream marker");
      ...
      if (!std::regex_search(tuple, std::regex(R"([^\x20-\x7E])"))) {
        ...
        kg_pipeline_stream_handler_logger.error(
            "Tuple contains non-printable data (hex): " + hexStream.str());
      }
  ...
  kg_pipeline_stream_handler_logger.info(
      "Closing connection for partitionId: " + std::to_string(partitionId));
}

bool Pipeline::streamGraphToDesignatedWorker(
    std::string host, int port, std::string masterIP, std::string graphId,
  ...
  kg_pipeline_stream_handler_logger.info("Connecting to worker Host:" + host +
  ...
  auto sendAndExpect = [&](const std::string& msg,
    ...
    if (bytes <= 0) {
      kg_pipeline_stream_handler_logger.error(
          "No response from worker for message: " + msg);
      return false;
    }
    ...
    if (resp != expected) {
      kg_pipeline_stream_handler_logger.error(
          "Unexpected worker response. Sent: " + msg +
          " Expected: " + expected + " Got: " + resp);
      return false;
    }
    kg_pipeline_stream_handler_logger.info("Worker responded '" + resp +
                                           "' for '" + msg + "'");
    return true;
  };
  ...
  for (JasmineGraphServer::worker worker :
       JasmineGraphServer::getWorkers(workerCount)) {
    counter++;
  ...
  if (continueKGConstruction) {
    ...
    long upload_bytes;
    std::string query =
        "SELECT uploaded_bytes FROM graph WHERE idgraph = " + graphId + ";";
    ...
    try {
      ...
      kg_pipeline_stream_handler_logger.info("uploaded bytes: " +
                                             std::to_string(upload_bytes));
  ...
  time_point previousTimeStampTriplesPerSecond =
  ...
  while (true) {
    ...
    if (msg == "meta") {
      ...
      Utils::send_str_wrapper(sockfd, JasmineGraphInstanceProtocol::OK);
      kg_pipeline_stream_handler_logger.info("Received content length: " +
      ...
      std::string sqlStatement =
          "UPDATE graph SET vertexcount = '" + graph["vertexcount"].dump() +
          "', centralpartitioncount = '" +
          graph["centralpartitioncount"].dump() + "', edgecount = '" +
          graph["edgecount"].dump() + "', graph_status_idgraph_status = '" +
          graph["graph_status_idgraph_status"].dump() + "' WHERE idgraph = '" +
          graphId + "'";
      ...
      for (auto partition : partitions) {
        ...
        //
        // if (result != SQLITE_OK) {
        //     kg_pipeline_stream_handler_logger.error("Failed to insert
        //     partition stats" + result); return false;
        // }
      }
      ...
      kg_pipeline_stream_handler_logger.info(
          "Worker completed streaming for graph " + graphId);
      break;
    } else {
      try {
        double completedBytes = std::stod(msg);
        // totalReceivedBytes += completedBytes;
        kg_pipeline_stream_handler_logger.info("Worker uploaded bytes: " +
                                               std::to_string(completedBytes));

        std::string updateQuery = "UPDATE graph SET uploaded_bytes = " +
                                  std::to_string(completedBytes) +
        ...
        double elapsed = static_cast<double>(elapsedSeconds.count());
        kgConstructionRates->bytesPerSecond =
            (completedBytes - previousBytesCount) / elapsed;
        ...
        if (*stopFlag) {
          ...
          *stopFlag = false;
          ...
          kg_pipeline_stream_handler_logger.info(
              "Stopping stream early as requested");
        ...
        kg_pipeline_stream_handler_logger.error(
            "Invalid progress message from worker: " + msg);
      }
    }
  }
  close(sockfd);
  kg_pipeline_stream_handler_logger.info(
      "Worker completed streaming upload for graph " + (graphId));
  return true;
}

...
class Pipeline {
 public:
  Pipeline(hdfsFS fileSystem, const std::string& filePath,
           int numberOfPartitions, int graphId,

           std::string masterIP, vector<JasmineGraphServer::worker>& workerList,
           std::vector<std::string> llmRunners);
  Pipeline(int connFd, hdfsFS fileSystem, const std::string& filePath,
           int numberOfPartitions, int graphId, std::string masterIP,
           vector<JasmineGraphServer::worker>& workerList,
           std::vector<std::string> llmRunners, std::string llmInferenceEngine,
           std::string llm, string chunkSize, std::string chunksPerBatch,
           long startFromBytes);
  void init();
  void startStreamingFromBufferToPartitions();

  static bool streamGraphToDesignatedWorker(
 ...
 private:
  ...
  json processTupleAndSaveInPartition(
      const std::vector<std::unique_ptr<SharedBuffer>>& tupleBuffer);
  void extractTuples(std::string host, int port, std::string masterIP,
...
std::vector<Chunk> split_document(const std::string& doc_id,
                                  const std::string& text,
                                  size_t chunk_words = 50);

// ---------- Pipeline components ----------
void document_reader(const std::string& docs_dir, TSQueue<Chunk>& chunk_q);
void worker_pool(size_t worker_id, TSQueue<Chunk>& chunk_q,
                 KafkaProducer& kprod, TSQueue<Triple>* local_out = nullptr);

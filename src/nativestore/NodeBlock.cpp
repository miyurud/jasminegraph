...
//        pthread_mutex_lock(&lockAddNodeProperty);
        if (newLink) {
            ...
            node_block_logger.log("blockaddr :" + std::to_string(newLink->blockAddress), "info");
...
//        pthread_mutex_unlock(&lockAddNodeProperty);
        ...
        // Problelm is Here.
        node_block_logger.log("XXXXXXXXX : Hi", "info");
...
bool NodeBlock::updateRelation(RelationBlock* newRelation, bool relocateHead) {
    unsigned int edgeReferenceAddress = newRelation->addr;  //relation address
    unsigned int thisAddress = this->addr;  //node address
...
std::list<NodeBlock> NodeBlock::getLocalEdges() {
...
std::list<NodeBlock> NodeBlock::getCentralEdges() {
    std::list<NodeBlock> edges;
    RelationBlock* currentRelation = this->getCentralRelationHead();
    while (currentRelation != NULL) {
        NodeBlock* node = NULL;
...
std::list<NodeBlock> NodeBlock::getAllEdges() {
    // Get local and central edges
    std::list<NodeBlock> localEdges = this->getLocalEdges();
    std::list<NodeBlock> centralEdges = this->getCentralEdges();

    // Concatenate local and central edges
    localEdges.insert(localEdges.end(), centralEdges.begin(), centralEdges.end());

    // Remove duplicates
    //localEdges.sort(); // Assuming NodeBlock has a comparison operator (operator<)
    //localEdges.unique(); // Remove consecutive duplicates
...
NodeBlock* NodeBlock::get(unsigned int blockAddress) {
    ...
    //if (strlen(label) != 0) {
        //id = std::string(label);
    //}
    nodeBlockPointer = new NodeBlock(std::to_string(nodeId), nodeId,blockAddress, propRef, edgeRef,centralEdgeRef, edgeRefPID, label, usage);

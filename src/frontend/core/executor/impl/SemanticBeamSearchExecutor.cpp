...
SemanticBeamSearchExecutor::SemanticBeamSearchExecutor(SQLiteDBInterface *db, PerformanceSQLiteDBInterface *perfDb,
...
void SemanticBeamSearchExecutor::execute()
{
    ...
    for (auto worker : workerList) {
        workerThreads.emplace_back(
            doSemanticBeamSearch,
            worker.hostname, worker.port,
            masterIP, std::stoi(graphId), count,
            queryString, std::ref(*bufferPool[count]),numberOfPartitions);
        count++;
    }
    vector<json> results;
    int closeFlag = 0;
    ...
    for (size_t i = 0; i < bufferPool.size(); ++i) {
        readThreads.emplace_back([&, i]() {
            semantic_beam_search_logger_executor.info("Starting read thread for bufferPool[" + std::to_string(i) + "]");
            while (true) {
                std::string data = bufferPool[i]->get();
                semantic_beam_search_logger_executor.info("Fetched data from bufferPool[" + std::to_string(i) + "]: " + data);
                if (data == "-1") {
    ...
    for (const auto& res : results)
    {
     ...
     semantic_beam_search_logger_executor.info("###CYPHER-QUERY-EXECUTOR### Executing Query : Fetching Results");



     semantic_beam_search_logger_executor.info("###CYPHER-QUERY-EXECUTOR### Executing Query : Completed");
...
void SemanticBeamSearchExecutor::doSemanticBeamSearch(std::string host, int port, std::string masterIP, int graphID,
                                               int partitionId, std::string query, SharedBuffer &sharedBuffer , int noOfPartitions)
{
    semantic_beam_search_logger_executor.info("Connecting to worker at " + host + ":" + std::to_string(port));
    semantic_beam_search_logger_executor.debug("Parameters: host=" + host + ", port=" + std::to_string(port) +
    ...
    semantic_beam_search_logger_executor.debug("Socket created, sockfd=" + std::to_string(sockfd));
    if (sockfd < 0) {
    ...
    if (host.find('@') != std::string::npos) {
        semantic_beam_search_logger_executor.debug("Host contains '@', splitting...");
    ...
    semantic_beam_search_logger_executor.debug("Attempting to connect to " + host + ":" + std::to_string(port));
    if (Utils::connect_wrapper(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        semantic_beam_search_logger_executor.error("Connection failed to " + host + ":" + std::to_string(port));
        close(sockfd);
        return ;
    }
    ...
    if (recv(sockfd, ack, strlen("stream-c-length-ack"), 0) <= 0) {
        semantic_beam_search_logger_executor.error("Failed to receive ACK after command");
        close(sockfd);
        return ;
    }
    ...
    semantic_beam_search_logger_executor.debug("Sending graphID length: " + std::to_string(ntohl(length)));
    send(sockfd, &length, sizeof(length), 0);
    semantic_beam_search_logger_executor.debug("Sending graphID: " + std::to_string(graphID));
    ...
    if (recv(sockfd, ack, strlen("stream-c-length-ack"), 0) <= 0) {
        semantic_beam_search_logger_executor.error("Failed to receive ACK after graphID");
        close(sockfd);
        return ;
    }
    ...
    semantic_beam_search_logger_executor.debug("Sending partitionId length: " + std::to_string(ntohl(length)));
    send(sockfd, &length, sizeof(length), 0);
    semantic_beam_search_logger_executor.debug("Sending partitionId: " + std::to_string(partitionId));
    ...
    bzero(ack, ACK_MESSAGE_SIZE);
    semantic_beam_search_logger_executor.debug("Waiting for ACK after partitionId");
    if (recv(sockfd, ack, strlen("stream-c-length-ack"), 0) <= 0) {
        semantic_beam_search_logger_executor.error("Failed to receive ACK after partitionId");
        close(sockfd);
        return ;
    }
    ...
    semantic_beam_search_logger_executor.debug("Sending query length: " + std::to_string(ntohl(length)));
    send(sockfd, &length, sizeof(length), 0);
    ...
    if (recv(sockfd, ack, strlen("stream-c-length-ack"), 0) <= 0) {
        semantic_beam_search_logger_executor.error("Failed to receive ACK after query");
        close(sockfd);
        return ;
    }
    ...
    string workers="";
    // --- Step 4: Send workers ---
    for (JasmineGraphServer::worker worker : JasmineGraphServer::getWorkers(noOfPartitions))
    {
        counter++;
        semantic_beam_search_logger_executor.info("count " + std::to_string(counter));
    ...
    semantic_beam_search_logger_executor.debug("Sending workers length: " + std::to_string(ntohl(length)));
    send(sockfd, &length, sizeof(length), 0);
    ...
    semantic_beam_search_logger_executor.info("Semantic Beam Search request sent successfully");
    while (true) {
        char start[ACK_MESSAGE_SIZE] = {0};
        recv(sockfd, &start, sizeof(start), 0);
        std::string start_msg(start);
        if (JasmineGraphInstanceProtocol::QUERY_DATA_START != start_msg) {
            semantic_beam_search_logger_executor.error("Error while receiving start command: " + start_msg);
            break;
        }
        ...
        if (return_status > 0) {
            ...
            semantic_beam_search_logger_executor.debug("Received int =" + std::to_string(content_length));
        } else {
            semantic_beam_search_logger_executor.error("Error while receiving content length");
            return ;
        }

...
void *uifrontendservicesesion(void *dummyPt) {
    ...
    while (!loop_exit) {
        ...
        } else if (token.compare(SHTRIAN) == 0) {
...
static void processTriangleCount(const std::string &masterIP, int connFd, SQLiteDBInterface *sqlite,
    ...
    ui_frontend_logger.info("Started processing triangle counting request " + reqId);
    ...
    if (threadPriority > Conts::DEFAULT_THREAD_PRIORITY) {
        ...
        graphSLA = JasmineGraphFrontEndCommon::getSLAForGraphId(sqlite, perfSqlite, graph_id, jobType,
